...
CREATE TRIGGER TR_HISTORY_RISICOREGEL ON RISICOREGEL AFTER UPDATE, DELETE AS BEGIN
	...
	BEGIN TRY
		...
		SELECT *, @Actie, dbo.FN_GET_NEW_VERSIENUMMER(D.PROJECTNUMMER, D.RAPPORTNUMMER, D.REGELNUMMER)
		FROM DELETED D
	END TRY
	BEGIN CATCH
...
CREATE TRIGGER TR_VISUELE_BEOORDELING_HISTORY ON VISUELE_BEOORDELING AFTER UPDATE, DELETE AS BEGIN
	...
	BEGIN TRY
		...
		SELECT *, @Actie, dbo.FN_GET_VERSIENUMMER(D.PROJECTNUMMER, D.RAPPORTNUMMER, D.REGELNUMMER)
		FROM DELETED D
	END TRY
	BEGIN CATCH
		IF @@TRANCOUNT > 0 BEGIN
