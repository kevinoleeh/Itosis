...
GO
/*======================================================================================*/
/* Alle stored procedures en triggers, zie voor extra toelichting het technisch ontwerp	*/
/*======================================================================================*/
...
CREATE PROCEDURE SP_INSERT_PROJECT
	@BEDRIJFSNAAM VARCHAR(255),
	@LOCATIE VARCHAR(255),
	@PROJECTOMSCHRIJVING VARCHAR(8000)
AS BEGIN
	-- Transaction level default omdat de procedure enkel een INSERT/UPDATE/DELETE doet.
...
CREATE PROCEDURE SP_INSERT_ORGANISATIE_RISICOREGEL
	...
	@RISICO_OMSCHRIJVING_OF_BEVINDING VARCHAR(8000),
	@HUIDIGE_BEHEERSMAATREGEL VARCHAR(255),
	@VOORGESTELDE_ACTIE_OF_VERBETERINGSMAATREGEL VARCHAR(8000),
	@AFWIJKENDE_ACTIE_TER_UITVOERING VARCHAR(255),
	@RESTRISICO VARCHAR(8000),
...
AS BEGIN
	...
	-- Repeatable read nodig om te voorkomen dat gegevens gewijzigd worden in:
	-- EXISTS
...
CREATE PROCEDURE SP_INSERT_MACHINEVEILIGHEID_RISICOREGEL
	...
	@AANVULLENDE_OMSCHRIJVING VARCHAR(8000),
	@TAKEN VARCHAR(8000),
	@TRANSPORT BIT,
	@MONTAGE BIT,
	@IN_BEDRIJFSNAME BIT,
	...
	@DEMONTAGE BIT,
	@ONTWERP VARCHAR(255),
	@AFSCHERMING VARCHAR(255),
	@INSTRUCTIE VARCHAR(8000),
...
CREATE PROCEDURE SP_INSERT_PLAN_VAN_AANPAK
	...
	@WERKINSTRUCTIE_PROCEDURE VARCHAR(8000),
	@TRA VARCHAR(255),
...
/*==============================================================*/
/*==============================================================*/
/* TRIGGER INSERT HISTORY RISICOREGEL	                        */
/*==============================================================*/
CREATE TRIGGER TR_HISTORY_RISICOREGEL ON RISICOREGEL AFTER UPDATE, DELETE AS BEGIN
	...
	BEGIN TRY
		...
		SELECT *, @Actie, dbo.FN_GET_RISICOREGEL_VERSIENUMMER(D.PROJECTNUMMER, D.RAPPORTNUMMER, D.REGELNUMMER)
		FROM DELETED D
	END TRY
	BEGIN CATCH
...
/*==============================================================*/
/* TRIGGER INSERT HISTORY VISUELE BEOORDELING                   */
/*==============================================================*/
CREATE TRIGGER TR_VISUELE_BEOORDELING_HISTORY ON VISUELE_BEOORDELING AFTER UPDATE, DELETE AS BEGIN
	...
	BEGIN TRY
		...
		SELECT *, @Actie, dbo.FN_GET_VISUELE_BEOORDELING_VERSIENUMMER(D.PROJECTNUMMER, D.RAPPORTNUMMER, D.REGELNUMMER)
		FROM DELETED D
	END TRY
	BEGIN CATCH
		IF @@TRANCOUNT > 0 BEGIN
...
/*==============================================================*/
/* TRIGGER INSERT HISTORY MACHINEVEILIGHEID                     */
/*==============================================================*/
CREATE TRIGGER TR_MACHINEVEILIGHEID_HISTORY ON MACHINEVEILIGHEID AFTER UPDATE, DELETE AS BEGIN
	...
	BEGIN TRY
		...
		SELECT *, @Actie, dbo.FN_GET_MACHINEVEILIGHEID_VERSIENUMMER(D.PROJECTNUMMER, D.RAPPORTNUMMER, D.REGELNUMMER)
		FROM DELETED D
	END TRY
	BEGIN CATCH
		IF @@TRANCOUNT > 0 BEGIN
...
/*==============================================================*/
/* TRIGGER INSERT HISTORY PLAN VAN AANPAK                       */
/*==============================================================*/
...
/*==============================================================*/
/* TRIGGER INSERT HISTORY PERIODIEKE BEOORDELING                */
/*==============================================================*/
...
/*==============================================================*/
/* TRIGGER MAXIMAAL AANTAL AFBEELDINGEN (3)		                */
/*==============================================================*/
CREATE TRIGGER TR_MAXIMAAL_AANTAL_AFBEELDING ON AFBEELDING AFTER INSERT, UPDATE AS BEGIN
	IF (@@ROWCOUNT = 0) BEGIN
		RETURN
	END
	...
	BEGIN TRY
		IF EXISTS (
			SELECT 1
			FROM AFBEELDING A INNER JOIN INSERTED I
			ON A.PROJECTNUMMER = I.PROJECTNUMMER
			AND A.RAPPORTNUMMER = I.RAPPORTNUMMER
			AND A.REGELNUMMER = I.REGELNUMMER
			HAVING COUNT(*) > 3
		) BEGIN
			RAISERROR('Maximaal aantal afbeeldingen (3) bereikt.', 16, 1)
		END
	END TRY
	BEGIN CATCH
		IF @@TRANCOUNT > 0 BEGIN
